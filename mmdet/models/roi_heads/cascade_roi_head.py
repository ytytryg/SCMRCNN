...
class CascadeRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def forward_dummy(self, x, proposals):
        ...
        if self.with_semantic:
            print("with_semantic")
            sem_pred = self.semantic_head(x)
        else:
            sem_pred = None   
            print("no_with_semantic")
    ...
    def simple_test(self, x, proposal_list, img_metas, rescale=False):
        ...
        if self.with_mask:
            if all(det_bbox.shape[0] == 0 for det_bbox in det_bboxes):
                ...
                _score = det_bboxes[0][:,4][...,None,None,None]     
        ...
        if self.with_semantic:
            ...
            sem_pred = F.interpolate(sem_pred, size=ori_shape, mode='bilinear', align_corners=self.semantic_head.align_corners)
            sem_pred = F.softmax(sem_pred, dim=1)
            if not self.test_cfg.is_logit:
                sem_pred = sem_pred.argmax(dim=1)
            sem_pred = sem_pred.cpu().numpy()   
            
            return list(zip(ms_bbox_result['ensemble'], ms_segm_result['ensemble'], sem_pred))   
