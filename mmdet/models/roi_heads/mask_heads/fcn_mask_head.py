...
class FCNMaskHead(BaseModule):
    ...
    def loss(self, mask_pred, mask_targets, labels, extra_loss=False):
        """
        Example:
        ...
        if extra_loss:
            loss['refine_loss_mask'] = loss_mask
        else:
            loss['loss_mask'] = loss_mask
        return loss

    def get_seg_masks(self, mask_pred, det_bboxes, det_labels, rcnn_test_cfg,
                      ori_shape, scale_factor, rescale,score=None):
        ...
        if rcnn_test_cfg.sem:
                mask_pred = mask_pred * score
        ...
        for inds in chunks:
           ...
           # print("inds",inds,"masks_chunk",masks_chunk)
            if rcnn_test_cfg.is_logit:
                mask_logit.append(masks_chunk.detach().cpu().numpy())
                
            else:
                if threshold >= 0:
                        masks_chunk = (masks_chunk >= threshold).to(dtype=torch.bool)
                else:
                   # mask_logit.append(masks_chunk)
